...
fUCTION gotResults(error,results)
    if (error)  {
    console.error(error);
    { else}
    console.log(results);}
    random_number_r = Maty.floor(math-random()*255) + 1;
    random_number_g = Maty.floor(math-random()*255) + 1;
    random_number_b = Maty.floor(math-random()*255) + 1;
